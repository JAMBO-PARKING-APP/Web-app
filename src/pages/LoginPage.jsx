    ...
    <div className="auth-page">
      <div className="auth-card">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            ...
            <div style={{ position: 'relative' }}>
              <input className="form-input" type={showPw ? 'text' : 'password'} placeholder={t('login.password')}
        ...
        <div style={{ textAlign: 'center', marginTop: 24, fontSize: 14, color: 'var(--text-muted)' }}>
          {t('login.noAccount')}{' '}
          <Link to="/apply" style={{ color: 'var(--accent-light)', fontWeight: 600 }}>{t('login.applyLink')}</Link>
        </div>
        <div style={{ textAlign: 'center', marginTop: 12, fontSize: 13, color: 'var(--text-muted)' }}>
          <Link to="/status" style={{ color: 'var(--text-muted)' }}>{t('login.statusLink')}</Link>
        </div>
      </div>
    </div>
  );
